...
void cu_run_color_level (CudaImg t_bgr_cuda_img, CudaImg t_bw_cuda_img, uchar3 color);
...
int main (int t_numarg, char** t_arg) {
    ...
    l_bgr_cuda_img.m_size.x = l_bw_cuda_img.m_size.x = l_bgr_cv_img.size ().width; // rozměry
    l_bgr_cuda_img.m_size.y = l_bw_cuda_img.m_size.y = l_bgr_cv_img.size ().height;
    l_bgr_cuda_img.m_p_uchar3 = (uchar3*) l_bgr_cv_img.data; // pointer na array s pixely
    ...
    cu_run_color_level (l_bgr_cuda_img, l_bw_cuda_img, color);
